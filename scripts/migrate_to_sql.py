...
async def init_fresh_db(force: bool = False):
    ...
    if db_path.exists():
        if force:
            logger.info(
                f"Force flag provided. Deleting existing database at {db_path}..."
            )
            os.remove(db_path)
            ...
            if response.lower() == "d":
                ...
                try:
                    async with aiosqlite.connect(db_path) as db:
                        ...
                        await db.commit()
                        logger.info(
                            "Successfully added 'session_id' column to learning_targets table."
                        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Initialize or migrate the TalkMate database."
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Force delete existing database and recreate",
    )
